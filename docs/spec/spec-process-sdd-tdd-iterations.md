...
| **2. Configurable fan-out** | Create named workflows with immutable endpoint versions; activate at most one version per workflow for new matching events under the provisional cardinality rule. A cutover routes later events to B while A's accepted work remains assigned to A. Explicit Disable pauses A's remaining work, and Resume does not reactivate it. | Routing, cutover, and pause/resume tests plus a UI walkthrough with two workflows and a replacement version. | Automatic retry implementation. |
| **3. Identity and trust** | The server assigns a queue ID when accepting a new event; resubmitting the same idempotency key and content returns that ID without new deliveries. Each event–endpoint pair is unique; receiver verifies a signed request. | Same-key duplicate/conflict, fresh-key identical-content, and signature-valid/tampered tests; one-time secret display. | Failure recovery and visual polish. |
