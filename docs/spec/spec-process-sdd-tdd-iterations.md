...
| **2. Configurable fan-out** | Create named workflows with immutable endpoint versions; activate at most one version per workflow for new matching events. A cutover routes later events to B while A's accepted work remains assigned to A. | Routing and cutover tests plus a UI walkthrough with two workflows and a replacement version. | Automatic retry and explicit Disable behavior after its spec decision. |
