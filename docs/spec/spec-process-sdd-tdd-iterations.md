...
| **2. Configurable fan-out** | Create multiple endpoints and subscriptions; a matching event creates a delivery only for each eligible endpoint; enable/disable affects new routing. | Routing tests and a UI walkthrough with two receivers. | Final policy for already-queued deliveries, retry. |
...
| **5. Operational experience** | Event-centered fan-out view updates without manual refresh and opens payload/attempt details; actions have loading, empty, and error states. | Browser-level flows for status change and investigation; accessibility and usability review. | Decorative motion and optional metrics. |
