...
- **USR-002 (Chosen direction):** The server assigns a unique event ID when a new event is accepted into the durable local queue. The sender does not choose this queue ID.
...
| **SPEC-004 Event identity** | **Queue ID chosen; submission-key semantics specified for review.** The local customer supplies a case-sensitive idempotency key of 1–128 ASCII letters, digits, `_`, or `-`, an event type of 1–64 such characters, and any valid JSON payload up to 32 KiB encoded as UTF-8. After validation and intake checks, the server generates an opaque unique event ID and atomically persists the new event and its deliveries before dispatch; this is entry into the durable local queue. The `201` response returns that event ID and `deduplicated=false`. A unique `(customer, idempotency key)` constraint identifies retries of the same intended submission. The server compares canonical JSON values, so object key order and whitespace do not change identity. Reusing a key with the same type and JSON value returns `200`, the **original** event ID, and `deduplicated=true`, with no new deliveries, even if the new-event rate limit is full. Reusing a key with changed type or JSON value returns `409 idempotency_conflict` without modifying the original. A different key creates a new event and ID even when type and payload are identical. A rejected request has no event ID. |
...
| Queue event identity (ID chosen; key behavior for review) | Server assigns a unique event ID at durable acceptance. Reusing a sender idempotency key with the same type/JSON value returns that original ID; changed content conflicts. A fresh key creates a new event even with identical content. | Separates the queue's identity from the sender's retry intent and avoids collapsing distinct identical occurrences. |
