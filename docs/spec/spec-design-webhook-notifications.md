...
| **SPEC-011 Limits and errors** | Local intake permits at most 60 new events per rolling minute for the fixed customer; exceeding it returns `429 rate_limited`. Invalid URL/ID/type/JSON/size returns `400 validation_error`. A conflicting event, unavailable replay, or attempt to activate a paused version returns 409 with `event_conflict`, `replay_unavailable`, or `endpoint_paused` respectively. Unexpected server errors use `500 internal_error` without secrets or payloads. API errors share `{code, message, details?}`; the frontend distinguishes these from transport failures and never invents delivery success. |
...
Specified edge outcomes to verify: changed payload under the same ID returns conflict; cutover A→B preserves A's outstanding deliveries; explicit Disable pauses queued work and blocks replay; malformed or oversized payload is rejected; unreachable/slow receivers follow the bounded retry policy; restart recovers expired claims; no-match events persist visibly; polling interruption shows stale state; a zero-denominator delivery rate shows `—`.
