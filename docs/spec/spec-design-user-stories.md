...
| --- | --- | --- |
| **US-01 Configure receivers** | As an operator, I create a named workflow, add immutable endpoint versions with URLs and event-type subscriptions, and activate one version for new events. A new secret appears once; later reads omit it. I can see which version replaced which. | **Partly agreed:** SPEC-002/003/005; AC-007/008/015. One version per workflow receives new events. Explicit Disable behavior remains open in SPEC-006. |
...
| --- | --- | --- |
| REQ-001 subscribed endpoints | US-01, US-03, US-11 | Partly agreed: SPEC-002/005; explicit Disable open |
...
| REQ-006 endpoint UI, secret once | US-01 | Partly agreed: SPEC-002/005; explicit Disable open |
...
| USR-001 named workflow and version lineage | US-01, US-11 | Agreed in principle: DEC-003, SPEC-002/005, AC-015 |

**Coverage result:** every assignment requirement and the newly agreed workflow relationship have a story. **Readiness result:** the explicit Disable and paused-replay outcomes are open again because routing cutover and an operator stop action need distinct meanings. The behavior draft is not final; tests and runtime evidence remain future work.

## 4. Decisions for owner review before the detailed plan

The [system spec's decision table](spec-design-webhook-notifications.md#11-validation-criteria-and-review-decisions) now records workflow/version cutover as agreed in principle and explicit Disable as the next decision. The remaining delivery, trust, operator, and proof choices still need sequential review. A change to any material choice updates its acceptance criterion and affected story before planning.
